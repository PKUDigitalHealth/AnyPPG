...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for batch_idx, (_, ppg_data, ecg_data) in enumerate(train_loader, start=skip_batches): 
                # Ensure we don't double count if global_step was advanced
                # (Logic handled by skip_batches above, but good to be safe)
                
                with self.accelerator.accumulate(self.model):
                    # Model forward
                    # outputs is a dict: {'loss_clip': ...}
                    outputs = self.model(ppg_data, ecg_data)
                    loss = outputs['loss_clip']
                    ...
                    # ecg_grad_norm should be 0 as it is frozen
                    ecg_grad_norm = self._compute_grad_norm(unwrapped_model.ecg_encoder.parameters())
                    
                    ppg_proj_grad_norm = self._compute_grad_norm(unwrapped_model.ppg_projector.parameters())
                    ecg_proj_grad_norm = self._compute_grad_norm(unwrapped_model.ecg_projector.parameters())
                    ...
                    if self.global_step % self.log_interval == 0 and self.accelerator.is_main_process:
                        ...
                        log_dict = {
                            "train/loss_clip": outputs.get('loss_clip', 0).item() if isinstance(outputs.get('loss_clip'), torch.Tensor) else 0,
                            "train/temperature": outputs.get('temperature', 0).item() if isinstance(outputs.get('temperature'), torch.Tensor) else 0,
                            "train/grad_norm_total": total_grad_norm,
                            "train/grad_norm_ppg": ppg_grad_norm,
                            "train/grad_norm_ecg": ecg_grad_norm,
                            "train/grad_norm_ppg_proj": ppg_proj_grad_norm,
                            "train/grad_norm_ecg_proj": ecg_proj_grad_norm,
                            "train/lr": current_lr,
                        ...
                        self.logger.info(
                            ...
                            f"CLIP: {log_dict['train/loss_clip']:.4f}, "
    ...
    def validate(self):
        ...
        with torch.inference_mode():
            for batch_idx, (_, ppg_data, ecg_data) in enumerate(self.valid_loader): 
                outputs = self.model(ppg_data, ecg_data)
                loss = outputs['loss']
                val_loss += loss.item()
                val_steps += 1
