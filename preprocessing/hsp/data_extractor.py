...
class HSPExtractor:
    def __init__(
        ...
        """Initialize HSP extractor with data paths and channel mapping.
    ...
    def process_all(
        ...
        """
        folders = list(self.dataset_dir.glob('*'))
        files = []
        for folder in tqdm(folders, total=len(folders), desc='Reading file paths'):
            stem = folder.stem
            if stem[:5] == 'sub-S':
                sessions = folder.glob('*')
                for session in sessions:
                    session_id = session.stem
                    file = session.joinpath('eeg', f'{stem}_{session_id}_task-psg_eeg.edf')
                    files.append(file)
