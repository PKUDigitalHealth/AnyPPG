#!/usr/bin/env python3
"""
HSP Data Processing Main Program - Enhanced Logging Version
Automatically capture output and exceptions for all HSPExtractor processing steps
"""
...
from data_extractor import HSPExtractor
...
def setup_logging(log_dir = "./preprocessing/hsp/logs") -> None:
...
def main() -> None:
    ...
    config = {
        "dataset_dir": "/path/to/datasets/HSP",
        "save_dir": "./preprocessing/hsp/HSP_Patient_Level",
        "map_dict": {"ECG-LA": "ECG", "Pleth": "PPG"},
        "num_workers": 32,
    }
    ...
    try:
        ...
        # Dynamically add logging decorator to HSPExtractor
        for name, method in vars(HSPExtractor).items():
            if callable(method) and not name.startswith("_"):
                setattr(HSPExtractor, name, log_method(method))
        
        # Execute processing
        extractor = HSPExtractor(**{k: config[k] for k in ['dataset_dir', 'save_dir', 'map_dict']})
