...
Automatically capture output and exceptions for all HSPProcessor processing steps
"""
...
from data_processor import HSPProcessor
...
def main() -> None:
    ...
    config = {
        "dataset_dir": './preprocessing/hsp/HSP_Patient_Level',
        "seg_duration": 60,
        "overlap_ratio": 0,
        "save_dir": './preprocessing/hsp/Proc_HSP_Patient_Level',
        "num_workers": 32,
        "pattern": '*.npz',
        "ppg_resample_sr": 50,
        "ecg_resample_sr": 125,
    ...
    try:
        ...
        # # Dynamically add logging decorator to HSPProcessor
        # for name, method in vars(HSPProcessor).items():
        #     if callable(method) and not name.startswith("_"):
        #         setattr(HSPProcessor, name, log_method(method))
        
        # Execute processing
        processor = HSPProcessor(**{k: config[k] for k in config.keys() if k != 'pattern'})
