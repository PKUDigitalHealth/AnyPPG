...
Automatically capture output and exceptions for all PulseDBProcessor processing steps
"""
...
from data_processor import PulseDBProcessor
...
def main() -> None:
    ...
    config = {
        "dataset_dir": './preprocessing/pulsedb/PulseDB_Patient_Level',
        "seg_duration": 10,
        "overlap_ratio": 0,
        "save_dir": './preprocessing/pulsedb/Proc_PulseDB_Patient_Level',
        "num_workers": 32,
        "pattern": '*.npz'
    }
    ...
    try:
        ...
        # # Dynamically add logging decorator to PulseDBProcessor
        # for name, method in vars(PulseDBProcessor).items():
        #     if callable(method) and not name.startswith("_"):
        #         setattr(PulseDBProcessor, name, log_method(method))
        
        # Execute processing
        processor = PulseDBProcessor(**{k: config[k] for k in config.keys() if k != 'pattern'})
