...
Automatically capture output and exceptions for all MESAExtractor processing steps
"""
...
from data_extractor import MESAExtractor
...
def main() -> None:
    ...
    config = {
        "dataset_dir": "/path/to/datasets/mesa/polysomnography/edfs",
        "save_dir": "./preprocessing/mesa/MESA_Patient_Level",
        "map_dict": {"EKG": "ECG", "Pleth": "PPG"},
        "num_workers": 32,
        "pattern": "*.edf"
    }
    ...
    try:
        ...
        # Dynamically add logging decorator to MESAExtractor
        for name, method in vars(MESAExtractor).items():
            if callable(method) and not name.startswith("_"):
                setattr(MESAExtractor, name, log_method(method))
        
        # Execute processing
        extractor = MESAExtractor(**{k: config[k] for k in ['dataset_dir', 'save_dir', 'map_dict']})
