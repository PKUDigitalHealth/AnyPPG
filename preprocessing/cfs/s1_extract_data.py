#!/usr/bin/env python3
"""
CFS Data Processing Main Program - Enhanced Logging Version
Automatically capture output and exceptions for all CFSExtractor processing steps
"""
...
from data_extractor import CFSExtractor
...
def setup_logging(log_dir = "./preprocessing/cfs/logs") -> None:
...
def main() -> None:
    ...
    config = {
        "dataset_dir": "/path/to/datasets/cfs/polysomnography/edfs",
        "save_dir": "./preprocessing/cfs/CFS_Patient_Level",
        "map_dict": {"ECG2": "ECG", "PlethWV": "PPG"},
        "num_workers": 32,
        "pattern": "*.edf"
    }
    ...
    try:
        ...
        # Dynamically add logging decorator to CFSExtractor
        for name, method in vars(CFSExtractor).items():
            if callable(method) and not name.startswith("_"):
                setattr(CFSExtractor, name, log_method(method))
        
        # Execute processing
        extractor = CFSExtractor(**{k: config[k] for k in ['dataset_dir', 'save_dir', 'map_dict']})
