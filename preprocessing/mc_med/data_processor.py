...
class MCMEDProcessor:
    ...
    def process_whole_dataset(self, pattern: str = '*.npz'):
        ...
        with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
            ...
            for future in tqdm(as_completed(tasks), total=len(tasks), desc='Processing MCMED data'):
                try:
    ...
    def process_individual_data(self, npz_path: str) -> Tuple:
        ...
        ppg_data = self.data_segmentor(ppg_data, self.seg_duration, ppg_sr, self.overlap_ratio, return_raw_windows=False)
        ecg_data = self.data_segmentor(ecg_data, self.seg_duration, ecg_sr, self.overlap_ratio, return_raw_windows=True)
    ...
    def data_reader(npz_path: str):
        ...
        ppg_data, ppg_sr = data['PPG'], data['PPG_metadata'][0]['sr']
        ecg_data, ecg_sr = data['ECG'], data['ECG_metadata'][0]['sr']

        return (ppg_data, ppg_sr), (ecg_data, ecg_sr)
    

    def data_segmentor(self, data: np.ndarray, seg_duration: float, sr: float, overlap_ratio: float = 0, return_raw_windows: bool = False):
        window_size = int(seg_duration * sr)
        step_size =int((1 - overlap_ratio) * window_size)
        windows = []
        for item in data:
            item_windows = self.slide_1d_window(item, window_size, step_size)
            if item_windows is not None:
                windows.append(item_windows)
        if len(windows) == 0:
            return
        stack_windows = np.vstack(windows)
        if return_raw_windows:
            return windows # windows reserved for invert detection
